...
namespace BlamCore.Cache.Base
{
    public abstract class CacheFile
    {
        ...
        public class StringTable : List<string>
        {
            ...
            public StringTable(Base.CacheFile Cache)
            {
                cache = Cache;
        ...
        public class IndexItem
        {
            ...
            public string ClassName
            {
                get
                {
                    if (Cache.Version >= CacheVersion.Halo3Beta)
                        return Cache.Strings.GetItemByID(Cache.IndexItems.ClassList[ClassIndex].StringID);

                    var xml = new MemoryStream();

                    switch (Cache.Version)
                    {
                        case CacheVersion.Halo1Xbox:
                        case CacheVersion.Halo1CE:
                        case CacheVersion.Halo1PC:
                            xml = new MemoryStream(Encoding.ASCII.GetBytes(BlamCore.Properties.Resources.Classes_H1));
                            break;

                        case CacheVersion.Halo2Xbox:
                            xml = new MemoryStream(Encoding.ASCII.GetBytes(BlamCore.Properties.Resources.Classes_H2));
                            break;
        ...
        public static XmlNode GetBuildNode(string build)
        {
            ...
            using (var xml = new MemoryStream(Encoding.ASCII.GetBytes(BlamCore.Properties.Resources.Builds)))
            {
        ...
        {
            XmlNode retNode = null;
            using (var xml = new MemoryStream(Encoding.ASCII.GetBytes(BlamCore.Properties.Resources.Versions)))
            {
                var xmlDoc = new XmlDocument();
                xmlDoc.Load(xml);
                var element = xmlDoc.DocumentElement;
        ...
        {
            XmlNode retNode = null;
            using (var xml = new MemoryStream(Encoding.ASCII.GetBytes(BlamCore.Properties.Resources.VertexBuffer)))
            {
                var xmlDoc = new XmlDocument();
                xmlDoc.Load(xml);
                var element = xmlDoc.DocumentElement;
