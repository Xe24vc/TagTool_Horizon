...
namespace BlamCore.Cache
{
    ...
    {
        public static Base.CacheFile GetCache(string Filename)
        {
            Base.CacheFile retCache = null;
            using (var fs = new FileStream(Filename, FileMode.Open, FileAccess.Read))
            {
                var Reader = new EndianReader((Stream)fs, EndianFormat.BigEndian);

                var head = Reader.ReadInt32();
                if (head == 1684104552) Reader.Format = EndianFormat.LittleEndian;
                ...
                var node = Base.CacheFile.GetBuildNode(build);
                switch (node.Attributes["definitions"].Value)
                {
                    ...
                    case "ReachBeta":   retCache = new HaloReachBeta.CacheFile(Filename, build); break;
                    case "ReachRetail": retCache = new HaloReachRetail.CacheFile(Filename, build); break;
