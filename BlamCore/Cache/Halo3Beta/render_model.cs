...
namespace BlamCore.Cache.Halo3Beta
{
    public class render_model : mode
    {
        ...
        public override void LoadRaw()
        {
            ...
            for (int i = 0; i < mode.ModelSections.Count; i++)
            {
                ...
                for (int j = 0; j < mode.VertInfoList[section.VertsIndex].VertexCount; j++)
                {
                    ...
                    MeshUtils.DecompressVertex(ref section.Vertices[j], mode.BoundingBoxes[0]);
                    ...
                    {
                        vert.Values.Add(new VertexValue(new RealQuaternion(section.NodeIndex, 0, 0, 0), VertexValue.ValueType.Int8_N4, "blendindices", 0));
                        vert.Values.Add(new VertexValue(new RealQuaternion(1, 0, 0, 0), VertexValue.ValueType.Int8_N4, "blendweight", 0));
                    }
                    #endregion
                    ...
                    if (mode.Flags.Values[18])
                    {
                        ...
                        if (!hasWeights) w = new VertexValue(new RealQuaternion(1, 0, 0, 0), VertexValue.ValueType.Int8_N4, "blendweight", 0);

                        if (vert.TryGetValue("blendindices", 0, out v))
                        {
                            v.Data.I = w.Data.I == 0 ? 0 : mode.NodeIndexGroups[i].NodeIndices[(int)v.Data.I].Index;
                            v.Data.J = w.Data.J == 0 ? 0 : mode.NodeIndexGroups[i].NodeIndices[(int)v.Data.J].Index;
                            v.Data.K = w.Data.K == 0 ? 0 : mode.NodeIndexGroups[i].NodeIndices[(int)v.Data.K].Index;
                            v.Data.W = w.Data.W == 0 ? 0 : mode.NodeIndexGroups[i].NodeIndices[(int)v.Data.W].Index;
                        }
                        else
                        {
                            v = new VertexValue(new RealQuaternion(0, 0, 0, 0), VertexValue.ValueType.Int8_N4, "blendindices", 0);
                            v.Data.I = mode.NodeIndexGroups[i].NodeIndices[0].Index;
                            vert.Values.Add(v);
                            vert.Values.Add(w);
                        }
                    }
        ...
        protected void LoadModelExtras()
        {
            ...
            for (int i = 0; i < part.Submeshes.Count; i++)
            {
                ...
                for (int j = submesh.SubsetIndex; j < (submesh.SubsetIndex + submesh.SubsetCount); j++)
                {
                    ...
                    var vList = MeshUtils.GetTriangleList(part.Indices, set.FaceIndex, set.FaceCount, mode.IndexInfoList[part.FacesIndex].FaceFormat);
                    ...
                    for (int k = 0; k < verts.Length; k++) // need to deep clone in case the vertices need to be           
                        verts[k] = part.Vertices[k + min]; // verts[k] = (Vertex)MeshUtils.DeepClone(); //transformed, so it doesnt transform all instances
            ...
            for (int i = 0; i < newRegion.Permutations.Count; i++)
            {
                ...
                for (int j = 0; j < modelPart.Vertices.Length; j++)
                {
                    ...
                    if (vert.TryGetValue("blendindices", 0, out v))
                        v.Data = new RealQuaternion(instance.NodeIndex, 0, 0, 0);
                    else
                        vert.Values.Add(new VertexValue(new RealQuaternion(instance.NodeIndex, 0, 0, 0), VertexValue.ValueType.Int8_N4, "blendindices", 0));

                    if (vert.TryGetValue("blendweight", 0, out w))
                        w.Data = new RealQuaternion(instance.NodeIndex, 0, 0, 0);
                    else
                        vert.Values.Add(new VertexValue(new RealQuaternion(1, 0, 0, 0), VertexValue.ValueType.Int8_N4, "blendweight", 0));
                }
            }
        ...
        new public class ModelSection : mode.ModelSection
        {
            ...
            {
                public Subset(Base.CacheFile Cache, int Address)
                {
                    EndianReader Reader = Cache.Reader;
                    Reader.SeekTo(Address);
        ...
        {
            public BoundingBox(Base.CacheFile Cache, int Address)
            {
                EndianReader Reader = Cache.Reader;
                Reader.SeekTo(Address);
        ...
        {
            public NodeIndexGroup(Base.CacheFile Cache, int Address)
            {
                EndianReader Reader = Cache.Reader;
                Reader.SeekTo(Address);
            ...
            {
                public NodeIndex(Base.CacheFile Cache, int Address)
                {
                    EndianReader Reader = Cache.Reader;
                    Reader.SeekTo(Address);
