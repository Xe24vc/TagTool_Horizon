...
namespace BlamCore.Cache.Halo1PC
{
    public class scenario_structure_bsp : sbsp
    {
            ...
            for (int i = 0; i < ModelSections.Count; i++)
            {
                ...
                for (int j = 0; j < section.Submeshes.Count; j++)
                {
                    ...
                    for (int k = 0; k < mesh.VertexCount; k++)
                    {
                        ...
                        var texcoord = new RealQuaternion(reader.ReadSingle(), 1f - reader.ReadSingle());
        ...
        {
            public Lightmap(Base.CacheFile Cache, int Address, int Magic)
            {
                var Reader = Cache.Reader;
            ...
            public class Material : mode.ModelSection.Submesh
            {
                ...
                public Material(Base.CacheFile Cache, int Address, int Magic) 
                {
                    var Reader = Cache.Reader;
                    Reader.SeekTo(Address);
