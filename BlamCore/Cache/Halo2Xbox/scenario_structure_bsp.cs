...
namespace BlamCore.Cache.Halo2Xbox
{
    public class scenario_structure_bsp : sbsp
    {
        public scenario_structure_bsp(Base.CacheFile Cache, Base.CacheFile.IndexItem Tag)
        {
            cache = Cache;
        ...
        public override void LoadRaw()
        {
            ...
            for (int i = 0; i < Clusters.Count; i++)
            {
                ...
                for (int j = 0; j < section.vertcount; j++)
                {
                    ...
                    var v = new Vertex();
                    var p = new RealQuaternion(reader.ReadSingle(), reader.ReadSingle(), reader.ReadSingle());
                ...
                for (int j = 0; j < section.vertcount; j++)
                {
                    ...
                    var uv = new RealQuaternion(reader.ReadSingle(), 1 - reader.ReadSingle());
        ...
        new public class Cluster : sbsp.Cluster
        {
            ...
            {
                XBounds = new Bounds<float>();
                YBounds = new Bounds<float>();
                ZBounds = new Bounds<float>();
        ...
        public class ModelSection : mode.ModelSection
        {
            ...
            public ModelSection(Base.CacheFile Cache, Base.CacheFile.IndexItem Tag, int Address, List<mode.BoundingBox> bounds)
            {
                EndianReader Reader = Cache.Reader;
                Reader.SeekTo(Address);
        ...
        {
            public InstancedGeometry(Base.CacheFile Cache, int Address, int modifier)
            {
                EndianReader Reader = Cache.Reader;
                Reader.SeekTo(Address);
