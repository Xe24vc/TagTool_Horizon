using BlamCore.IO;
using play = BlamCore.Cache.cache_file_resource_layout_table;
...
namespace BlamCore.Cache.Halo4Beta
{
    public class cache_file_resource_layout_table : Halo3Retail.cache_file_resource_layout_table
    {
        ...
        public cache_file_resource_layout_table(Base.CacheFile Cache, int Address)
        {
        ...
        {
            public Segment(Base.CacheFile Cache, int Address)
            {
                EndianReader Reader = Cache.Reader;
                Reader.SeekTo(Address);
